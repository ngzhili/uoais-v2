...
if __name__ == "__main__":
    ...
    for i, (rgb_path, depth_path, anno_path) in enumerate(zip(tqdm(rgb_paths), depth_paths, vis_anno_paths)):
        ...
        # occlusion_mask_list = []
        occ_annos = [0 for _ in range(len(filtered_amodal_paths))]
        for anno_path in filtered_occlusion_paths:
            # get instance id  
            inst_id = os.path.basename(anno_path)[:-4].split("_")[-1]
            inst_id = int(inst_id)
            # load mask image
            ...
            # anno_mask[cnd] = inst_id
            try:
                occ_annos[inst_id-1] = 1
            except:
                print("inst_id-1 is out of range",inst_id-1)   
                print("len(filtered_amodal_paths)",len(filtered_amodal_paths))     
        ...
        # print('preds:\n',preds)
        # print('pred_occs:\n',pred_occs)
        # print('pred_occluded_masks:\n',pred_occluded_masks)
        # print('bboxes:\n',bboxes)

        

        # for i,mask in enumerate(preds):
        #     vis_save_path = os.path.join(output_dir,rgb_path.split("/")[-1][:-4]+f'_amodal_{i}.png')
        #     # cv2.imwrite(vis_save_path, np.float32(mask))
        #     plt.figure()
        #     plt.imshow(mask)
        ...
        #     vis_save_path = os.path.join(output_dir,rgb_path.split("/")[-1][:-4]+f'_visible_{i}.png')
        #     # cv2.imwrite(vis_save_path, np.float32(mask))
        #     plt.figure()
        #     plt.imshow(mask)
        #     plt.savefig(vis_save_path)
        ...
        # for i,mask in enumerate(pred_occluded_masks):
        #     vis_save_path = os.path.join(output_dir,rgb_path.split("/")[-1][:-4]+f'_occlusion_{i}.png')
        #     # cv2.imwrite(vis_save_path, np.float32(mask))
        #     plt.figure()
        #     plt.imshow(mask)
        #     plt.savefig(vis_save_path)
